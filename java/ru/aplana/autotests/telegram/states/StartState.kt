...
class StartState(userContext: UserContext, chatId: Long?, send: (SendMessage, Boolean) -> Unit) : UserState(userContext, chatId, send) {
    ...
    override fun onMessageReceived(message: Message) {
        ...
        when (messageText) {
            ...
            "Редактировать проекты" -> userContext.changeState(ProjectManagementState(userContext, chatId, send))
            else -> {
    ...
    override fun showFirst() {
        val buttons = mutableListOf("Зарегистрировать проект", "Подписаться на проект", "Редактировать проекты")
